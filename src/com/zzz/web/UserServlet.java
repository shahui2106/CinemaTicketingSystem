...
//        2、重定向到首页（或登录页面）。
    ...
    protected void login(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        User loginUser = userService.login(new User(username, StringUtils.string2MD5(password), null, "0"));
        // 如果等于null,说明登录 失败!
        if (loginUser == null) {
...
//        3、用户名是否存在
            if (userService.existsUsername(username)) {
                ...
                if (res) {
                    ...
                    req.setAttribute("username", username);
                    req.setAttribute("phone", phone);
                    req.getRequestDispatcher("/pages/user/forgetpassword.jsp").forward(req, resp);
                }
            } else {
                //没有此用户名
