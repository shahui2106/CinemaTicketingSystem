...
var Alert = {
    ...
    showMsg: function (obj) {
        var subhtml = '<div id="alert_dialog_show_msg_box" style=" overflow:hidden;height:100px;width:200px;margin:-32% auto;"><div class="sweet-overlay" tabIndex="-1" style=" background-color:#000; opacity:0.4; position: fixed; left: 0; right: 0; top: 0; bottom: 0; z-index:1000;"></div><div id="alert_show_3" style="height:100px; width:200px; background-color:#fff; color:#000; border-radius:8px;font-size:20px; text-align:center;z-index:2000;position:absolute;"><p style="font-size:14px; margin-top:20px;">' + obj + '</p><input name="button" onclick="closedShowMsg()" type="button" value="确认" style=" width: 65px; height: 32px; line-height: 30px; font-size: 14px; border-radius: 5px; background-color: #67DAC8; color: white; margin-top: 16px;"  /></div></div>';
        $("body").append(subhtml);
    },
    showConfirmMsg: function (obj, callback) {
        var subhtml = '<div id="alert_dialog_show_confirm_box" style=" overflow:hidden;height:100px;width:200px;margin:-32% auto;"><div class="sweet-overlay" tabIndex="-1" style=" background-color:#000; opacity:0.4; position: fixed; left: 0; right: 0; top: 0; bottom: 0; z-index:1000;"></div><div id="alert_show_3" style="height:100px; width:200px; background-color:#fff; color:#000; border-radius:8px;font-size:20px; text-align:center; position:absolute;z-index:2000;"><p style="font-size:14px; margin-top:20px;">' + obj + '</p><input name="button" onclick="rec(' + callback + ')" type="button" value="确认" style="width: 65px; height: 32px; line-height: 30px; font-size: 14px; border-radius: 5px; background-color: #67DAC8; color: white; margin-top: 16px;"  /><input name="button" onclick="rec()" type="button" value="取消" style=" width: 65px; height: 32px; line-height: 30px; font-size: 14px; border-radius: 5px; background-color: #67DAC8; color: white; margin-top: 16px;"  /></div></div>';
        $("body").append(subhtml);
